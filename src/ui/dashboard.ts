...
<body>
  ...
  (() => {
    ...
    const renderStateMachine = () => { const section = el("section", { class: "card", "aria-label": "Programming state machine" }); section.append(el("h2", {}, "Programming state")); const steps = el("div", { class: "state-machine" }); [["Proposed / Preview", "M1: protected records and deterministic dry-run", "active"], ["Approved", "No approval action is exposed by the current M1 API", "unavailable"], ["Published (Versioned)", "M4 publication boundary; no published version is available", "unavailable"], ["Executed (Runtime)", "Runtime controls are unavailable and not rendered here", "unavailable"]].forEach((step) => { const item = el("div", { class: "state-step " + step[2] }); item.append(el("strong", {}, step[0]), el("span", {}, step[1])); steps.append(item); }); section.append(steps); return section; };
