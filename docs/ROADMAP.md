...
|  **M3**   | Ingestion & Asset Lifecycle | M3.1–M3.11 complete; M3.12 two-phase exception pending | Not started operationally | **Completed 2026-08-04** | M3.12 execution acceptance before M4.1 |
...
not revoke that acceptance. Both phases remain separately gated: no M3
migration, worker, media intake, provider connection, or operational activation
has occurred.
...
- **Authoritative record:** [M3.12 operational staging activation authorization
  ...
  defines the sole narrowly bounded post-acceptance exception path.
- **Purpose / planes / mode / prerequisites:** Define two strictly ordered
  ...
  dependent on M3.10/M3.11 and accepted M2 authorization/DR decisions.
...
- **Controls / evidence / gate:** Require `id + station_id`, `not_found`,
  ...
  No approval creates standing authority. M4.1 remains deferred until M3.12
...
execution. All M4 work is unstarted and requires a new approval.
...
- **Controls / evidence / gate:** Validate station-scoped IDs, legal state
  transitions, content-free audit vocabulary, and rollback semantics. Owner
  approval is required before M4.2.
